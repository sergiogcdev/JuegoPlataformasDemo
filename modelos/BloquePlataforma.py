...
class BloquePlataforma(Modelo):
	def __init__(self, screen, loc):
		super().__init__(screen, 'res/boxes_28x24.png', [loc[0], loc[1]], [28, 24] )
		self.pair = [56, 0]
		self.surface = pygame.image.load(self.img_path).convert_alpha()
	def obtener_elemento(self):
		self.surface.set_clip(pygame.Rect(self.pair[0], self.pair[1], 28, 24))
